...
`--lint` を付けると、推奨規約に基づく heuristic lint も実行する。
`--strict-lint` を付けると lint 指摘がある場合も exit code 1 にする。
...
#### `boxel palette <subcommand>`

トップレベル `palette` を操作する。

```bash
boxel palette list myhouse.boxel.json
boxel palette add myhouse.boxel.json --name main-wall --color "#E7EBF2" --description "主壁"
boxel palette update myhouse.boxel.json --name main-wall --color "#FFFFFF"
boxel palette remove myhouse.boxel.json --name main-wall
```
...
boxel ortho shrine.boxel.json --crop structure --center --grid 5
boxel ortho shrine.boxel.json --highlight-color "#20314E"
```

`--mode coord` では、各セルに可視面の座標値を表示する。`--style braille` を付けると 9 段階の点字ヒートマップとして描画する。
`--crop structure` は structure の範囲だけに切り詰める。`--center` と `--grid` は補助線を重ねる。`--highlight-color` は指定色を強調表示する。
